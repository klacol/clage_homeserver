...
def mocked_requests_get(*args, **kwargs):
    ...
    if args[0] == 'http://127.0.0.1/status':
...
class TestClageWaterHeater(TestCase):
    ...
    def test_requestStatus(self, mock_get):
        status = ClageWaterHeater('http://127.0.0.1/status','MyHomeServerID').requestStatus()
    ...
    def test_setAccessType(self, mock_get):
        ...
        mock_get.assert_called_once_with('http://127.0.0.1/status')
